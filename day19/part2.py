...
print(984, get_min_max(984))
print(1084, get_min_max(1084))

for y in range(900, 1200):
    min_x, max_x = get_min_max(y, min_x, max_x)
    width[y] = (min_x, max_x)
    #print(y, min_x, max_x)
...
for y in range(1000, 1200):
    if width[y-99][1] >= width[y][0] + 99:
        result = (y-99, width[y][0])
        print(result)
        break

for y in range(result[0]-5, result[0]+100+5):
    for x in range(result[1]-5, result[1]+100+5):
        if is_x_y_affected(x, y):
            if y < result[0] or y >= result[0]+100 or x < result[1] or x >= result[1]+100:
                sys.stdout.write('#')
            else:
                sys.stdout.write('*')
        else:
